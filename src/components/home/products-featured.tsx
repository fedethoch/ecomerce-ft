  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const list = normalized.filter((p) => p.isOutstanding)
        if (mounted) setFeatured(limit ? list.slice(0, limit) : list)
      } catch (err: any) {
        if (mounted) setError(err?.message ?? "No se pudieron cargar los destacados")
      } finally {
      ...
      <section className={cn("w-full bg-background py-16", className)}>
        <div className="max-w-full mx-auto px-4">
          ...
          <div className="bg-card/60 backdrop-blur-sm rounded-2xl p-12 border border-border text-center">
            <p className="text-[#6B7280] text-lg">No hay productos destacados por ahora.</p>
          </div>
        </div>
      </section>
    )
