...
export function ProductCard({ product, relatedProducts, onAddToCart }: ProductCardProps) {
          ...
          {relatedProducts.map((relatedProduct) => (
            <Card key={relatedProduct.id} className="group cursor-pointer hover:shadow-lg transition-shadow">
              <CardContent className="p-4">
                ...
                <div className="flex items-center gap-1 mb-2">{renderStars(relatedProduct.rating)}</div>
                <p className="font-bold text-lg">${relatedProduct.price.toLocaleString()}</p>
              </CardContent>
            </Card>
          ))}
        </div>
