...
interface ProductsViewProps {
  setActiveView: (view: string, productId?: string) => void // Modificado para aceptar ID
}

export function ProductsView({ setActiveView }: ProductsViewProps) {
  ...
  const handleEditProduct = (id: string) => {
    // Cambiar a la vista de edición con el ID del producto
    setActiveView("edit-product", id)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Productos</h1>
          <p className="text-muted-foreground">Gestiona tu inventario de ropa ({products.length} productos)</p>
        </div>
        <Button onClick={() => setActiveView("create-product")}>
